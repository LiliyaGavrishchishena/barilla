var Moolidator_Lite = new Class({
  ...
  options: {
    ...
    single_input_fields_emoticon_check: [
      'email',
      'password',
      'tel',
      'text',
    ...
    browser: {
      mobile: {
        ...
        iOsWebview: /(iPhone|iPod|iPad).*AppleWebKit(?!.*Version)/.test(
          navigator.userAgent
        ),
      },
      desktop: {
  ...
  global_functions: {},
  global_version: { name: 'moolidator_lite', version: '1.104' },
  ...
  initialize: function (options) {
    ...
    if (Object.getLength(options.rules) > 0) {
      ...
      var country_select = $$(
        'select[name=' + _self.options.country_name + ']'
      );
  ...
  initialize_recaptcha: function (recaptcha) {
    ...
    var patt = /^{{/;
    if (
      _self.options.recaptcha.data['data-sitekey'] != '' &&
      patt.test(_self.options.recaptcha.data['data-sitekey']) == false
    ) {
      ...
      if (typeOf(submit_button) == 'element') {
        ...
        var merge = Object.merge(
          _self.options.hidden_fields,
          _self.options.recaptcha.hidden_fields
        );
        _self.options.hidden_fields = merge;
        _self.options.recaptcha.active = true;
        _self.console_log(
          '# ReCaptcha injected : ' + _self.options.submit_button_id
        );
      }
    } else {
  ...
  scan_fields: function () {
    ...
    $$('.' + _self.options.validate_class).each(function (field) {
      if (field.get('tag') == 'input') {
        if (
          _self.options.single_input_fields.contains(field.get('type')) == true
        ) {
          fields[field.get('name')] = field;
        } else {
          if (
            _self.options.multiple_input_fields.contains(field.get('type')) ==
            true
          ) {
        ...
        click: function (event) {
          if (_self.get_event_target(event).hasClass('ignore') == false) {
            if (_self.is_form_valid(event) == true) {
              if (
                _self.options.recaptcha.active === true &&
                typeOf(window[_self.options.recaptcha.recaptcha_function]) ==
                  'object' &&
                $$('.' + _self.options.recaptcha.recaptcha_key_error_class)
                  .length == 0
              ) {
  ...
  add_events_to_country: function (element) {
    ...
    element.addEvents({
      change: function () {
        ...
        _self.console_log(
          '##### country set to: ' + _self.global_selected_country
        );
        _self.validate_form();
      },
    });
  ...
  add_events_to_element: function (element) {
    ...
    if (element.get('tag') == 'input') {
      if (
        _self.options.single_input_fields.contains(element.get('type')) == true
      ) {
        element.addEvents({
          ...
          paste: function (event) {
            if (
              document.cookie.match(/oip=on/) === null &&
              _self.options.allow_paste === false
            ) {
        ...
        if (
          _self.options.multiple_input_fields.contains(element.get('type')) ==
          true
        ) {
          element.addEvents({
            click: function (event) {
  ...
  add_hidden_fields_to_form: function () {
    ...
    Object.each(_self.options.hidden_fields, function (value, field_name) {
      if ($$('input[name="' + field_name + '"]').length == 0) {
        ...
        _self.console_log(
          '# add_hidden_fields_to_form not, field exist: ' + field_name
        );
      }
    });
  },
  ...
  add_function: function (function_name, function_position, function_code) {
    var _self = this;
    _self.global_functions[
      function_name + '_' + function_position
    ] = function_code;
    _self.console_log(
      '### add_function - name: ' +
        function_name +
  ...
  attempt_function: function (element, function_position) {
    ...
    if (Object.keys(_self.global_functions).contains(function_name) == true) {
      if (function_position == _self.options.function_before_addition) {
        _self.global_functions[
          element.get('name') + _self.options.function_before_addition
        ].attempt(element);
      } else {
        _self.global_functions[
          element.get('name') + _self.options.function_after_addition
        ].attempt(element);
      }
    ...
    if (
      Object.keys(_self.global_functions).contains(all_function_name) == true
    ) {
  ...
  on_element_keyup: function (element, event) {
    var _self = this;
    if (
      typeOf(element.ml_validate) != 'null' &&
      _self.is_key_pressed(9, event) == false
    ) {
      _self.is_field_filled(element);
      if (element.get('tag') == 'input') {
  ...
  removeEmoticonFromInputField: function () {
    ...
    $$('input').each(function (element) {
      if (
        _self.options.single_input_fields_emoticon_check.contains(
          element.get('type')
        ) === true
      ) {
        element.addEvents({
    ...
    if (_self.options.check_date == true) {
      if (_self.options.date_fields.contains(element.get('name'))) {
        ...
        if (rule_result < _self.static_result_invalid) {
          if (
            $$('[name=' + _self.options.date_fields[1] + ']')[0].ml_result >
            _self.static_result_not_validated
          ) {
            _self.set_element_invalid(
              $$('[name=' + _self.options.date_fields[1] + ']')[0],
              false
            );
          }
          if (
            $$('[name=' + _self.options.date_fields[0] + ']')[0].ml_result >
            _self.static_result_not_validated
          ) {
            _self.set_element_invalid(
              $$('[name=' + _self.options.date_fields[0] + ']')[0],
              false
            );
          }
        } else {
  ...
  on_input_label_blur: function (element, event) {
    ...
    if (typeOf(placeholder) != 'null' && placeholder != '') {
      if (
        element.get('value') == '' ||
        element.get('value').toLowerCase() == placeholder.toLowerCase()
      ) {
  ...
  validate_form: function (only_validated) {
    var _self = this;
    _self.console_log(
      '##### validate_form: begin - only_validated: ' + only_validated
    );
    var fields = _self.scan_fields();
    Object.each(fields, function (elements) {
      if (typeOf(elements) == 'element') {
    ...
    if (use_rules != false) {
      if (checked_element.length > 0) {
        Object.each(use_rules.positive, function (rule) {
          ...
          var rule_result = _self.get_rule_result(
            check_element.get('value'),
            rule,
            use_rules.trimming,
            true
          );
          ...
          _self.console_log(
            '# validate_radio_checkbox - name: ' +
              check_element.get('name') +
              ' - value: ' +
              check_element.get('value') +
              ' - use_rule: ' +
              rule +
              ' - optional: ' +
              element.hasClass(_self.options.optional_class) +
              ' - result: ' +
              rule_result
          );
        });
      ...
      _self.console_log(
        '# validate_radio_checkbox - name: ' +
          checked_element[0].get('name') +
          ' - no rule set'
      );
    }
    $$('[name=' + checked_element[0].get('name') + ']').each(function (
      radio_checkbox
    ) {
  ...
  validate_text: function (element) {
    ...
    _self.attempt_function(element, _self.options.function_before_addition);
    if (
      _self.options.input_label == true &&
      element.get('value').trim() != ''
    ) {
    ...
    if (use_rules != false) {
      ...
      if (result == _self.static_result_invalid) {
        if (
          element.hasClass(_self.options.optional_class) == true &&
          element.get('value').trim() == ''
        ) {
          result = _self.static_result_optional;
        }
      ...
      if (_self.options.input_label == false) {
        result = _self.static_result_no_rule;
        _self.console_log(
          '# validate_text - name: ' + element.get('name') + ' - no rule set'
        );
      } else {
        if (typeOf(placeholder) == 'null' || placeholder == '') {
          result = _self.static_result_no_rule;
  ...
  validate_select: function (element) {
    ...
    if (use_rules != false) {
      ...
      result = _self.static_result_no_rule;
      _self.console_log(
        '# validate_select - name: ' + element.get('name') + ' - no rule set'
      );
    }
    element.ml_result = result;
    _self.process_validation(element, result);
  ...
  validate_textarea: function (element) {
    ...
    if (use_rules != false) {
      ...
      result = _self.static_result_no_rule;
      _self.console_log(
        '# validate_textarea - name: ' + element.get('name') + ' - no rule set'
      );
    }
    element.ml_result = result;
    _self.process_validation(element, result);
  ...
  validate_recaptcha: function (token) {
    ...
    if (token == $(_self.options.recaptcha.recaptcha_response_field_id).value) {
      $(_self.options.recaptcha.recaptcha_hidden_field_id).set(
        'value',
        _self.check_recaptcha_puzzle()
      );
      _self.console_log('##### submit form: true');
  ...
  check_recaptcha_puzzle: function () {
    ...
    if (
      Math.round(
        new Date().getTime() - _self.options.recaptcha.recaptcha_start
      ) > 2000
    ) {
      _self.options.recaptcha.puzzle_visible = true;
    }
    _self.console_log(
      '# ReCaptcha puzzle visible: ' + _self.options.recaptcha.puzzle_visible
    );
  ...
  get_date_result: function (element) {
    ...
    date_data[0] = parseInt(
      $$('[name=' + _self.options.date_fields[2] + ']')[0].get('value')
    );
    date_data[1] = parseInt(
      $$('[name=' + _self.options.date_fields[1] + ']')[0].get('value')
    );
    date_data[2] = parseInt(
      $$('[name=' + _self.options.date_fields[0] + ']')[0].get('value')
    );
  ...
  get_grouped_elements: function (element) {
    ...
    if (status.length > 0) {
      status[0]
        .get('class')
        .split(' ')
        .each(function (status_class) {
          if (
            status_class.search(
              new RegExp(
                '^' + _self.options.status_class_addition + '\\w+$',
                'i'
              )
            ) > -1
          ) {
            var element_name = status_class.replace(
              new RegExp(
                '^' + _self.options.status_class_addition + '(\\w+)$',
                'i'
              ),
  ...
  is_form_valid: function (event) {
    ...
    $$('input').each(function (emo_element) {
      if (
        _self.options.single_input_fields_emoticon_check.contains(
          emo_element.get('type')
        ) === true
      ) {
        if (emo_element.value && emo_element.value.length > 0) {
          emo_element.value = emo_element.value.replace(
            /(\u00a9|\u00ae|[\u2000-\u3300]|\ud83c[\ud000-\udfff]|\ud83d[\ud000-\udfff]|\ud83e[\ud000-\udfff])/g,
            ''
          );
        }
  ...
  is_radio_check_optional: function (element) {
    ...
    if (
      element.hasClass(_self.options.optional_class) == true &&
      $$('[name=' + element.get('name') + ']:checked').length == 0
    ) {
      result = true;
    }
    return result;

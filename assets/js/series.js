...
var field_validator, current_page, mooli, scroll_position_x;
...
var iframe_selector = new Class({
  Implements: Options,
  options: { target_window: 'popped_iframe' },
  regex_pdf: /\.pdf$/,
  ...
  setup_selector: function (element, type) {
    var _self = this;
    if (
      element.hasClass('advertiselink') ||
      element.hasClass('ignore') ||
      element.href.test(_self.regex_javascript, 'i')
    )
      return;
    if (element.href) {
...
var open_layer = function (element, href) {
  ...
  var req = new Request({
    ...
    onSuccess: function (response) {
      ...
      if (!$('popped_layer')) {
        var popped_layer = new Element('div', { id: 'popped_layer' });
      ...
      $$('#popped_layer a').each(function (element) {
        if (
          element.href.test(/self.close()/) == true ||
          element.href.test(/window.close()/) == true
        ) {
          element.dispose();
        }
      });
      ...
      $('pop_iframe').setStyle(
        'background-color',
        $('popped_layer').getStyle('background-color')
      );
      $$('#popped_layer .poplink, #popped_layer .sponsoren_iframe').each(
...
var setFilledClass = function () {
  $$('[id^=form_reg]')
    .getElements('input[type=text]:not(.validate)')
    .each(function (el) {

window.addEvent('domready', function () {
  ...
  appendScript(
    './assets/js/s5-bfp.js?' + Math.round(Math.random() * 10000000000) + '',
    ['_reg_half', '_reg_full']
  );
...
var Maxlength_on_number_fields = (function () {
  ...
  function get_fields() {
    return $$(
      'input[type=number][maxlength], input[type=number][data-maxlength]'
    ).map(function (field) {
      return {
        el: field,
...
var remove_matched_from_localstorage = function (matchStr, keepParam) {
  if (localStorage_is_supported && localStorage.length > 0) {
    ...
    for (var i = 0; i < localStorage.length; i++) {
      if (
        localStorage.key(i).match(regex) &&
        localStorage.key(i) !== keepParam
      ) {
...
var popmaster = (function () {
  ...
  function init() {
    ...
    FMiKauYpzhc.Logger.log(
      '### Browser supports real popunder: ' +
        FMiKauYpzhc.Browser.popunderAvailable()
    );
    FMiKauYpzhc.Logger.log(' ');
  ...
  function add_pop(pop) {
    ...
    if (pop_options !== undefined && $(pop.trigger) !== null) {
      ...
      FMiKauYpzhc.Logger.log('');
      if (pop.type !== 'body_leave')
        FMiKauYpzhc.add(pop.url, pop_options).bindTo(pop.trigger);
    }
  }
  function get_pop_options(pop) {
    var pop_options = {
      ...
      shouldFire: function (should, FMiKauYpzhc_object) {
        ...
        var should_fire =
          trigger_clicked(pop) === true &&
          check_conditions(pop) === true &&
          check_localStorage(pop) === true;
        ...
        if (should_fire) {
          try {
            if (
              pop.trigger.length &&
              pop.trigger[0].id !== undefined &&
              pop.trigger[0].id.indexOf('submit_reg_') > -1
            ) {
              if (
                typeof mooli !== undefined &&
                mooli.is_form_valid() === false
              ) {
                should_fire = false;
              }
            }
      ...
      beforeOpen: function (url, FMiKauYpzhc_object) {
        FMiKauYpzhc.Logger.log(
          '### Firing ' +
            pop_under_objects[FMiKauYpzhc_object.name].type +
            ' PK ' +
            FMiKauYpzhc_object.name +
            ' with URL ' +
            url
        );
        try {
      ...
      afterOpen: function (url, FMiKauYpzhc_object) {
        try {
          if (context == 'br' && 'page_reg_full' == $$('body').get('id')) {
            log.send({
              url: '/cgi-bin/global.pl',
              params: { ident: 'popunder_regfull_sum', todo: 'log_misc' },
            });
          }
        } catch (e) {}
        setPopupReappearDelay(FMiKauYpzhc_object.name);
        FMiKauYpzhc.Logger.log(
          '### Trigger clicked:',
          pop_under_objects[FMiKauYpzhc_object.name].trigger[0]
        );
    ...
    if (
      pop.type == 'popunder' &&
      FMiKauYpzhc.Browser.popunderAvailable() === false
    ) {
      pop.type = 'tabunder';
    }
    if (
      $(document.body).id.match(/multicoreg/) !== null &&
      pop.type == 'tabunder'
    ) {
      pop.type = 'newtab';
    }
  ...
  function check_conditions(pop) {
    return (
      check_targeting(pop) === true &&
      is_in_iframe(pop) === false &&
      was_fired(pop) === false &&
      check_age_range(pop)
    );
  }
  function check_localStorage(pop) {
    var page_info =
      ...
      pop.id +
      '_' +
      $$('#wingame_pk').get('value') +
      $$('#freetest_pk').get('value');
  ...
  function check_age_range(pop) {
    ...
    var years_diff =
      current_date.getFullYear() - current_user_birthday.getFullYear();
    if (
      current_date.getMonth() < current_user_birthday.getMonth() ||
      (current_date.getMonth() == current_user_birthday.getMonth() &&
        current_date.getDate() < current_user_birthday.getDate())
    ) {
      years_diff--;
    }
  ...
  function get_current_user_birthday() {
    ...
    try {
      if (
        ...
        $('monthofbirth') &&
        $('yearofbirth') &&
        $('yearofbirth').get('value') &&
        $('monthofbirth').get('value') &&
        $('dayofbirth').get('value')
      ) {
        current_user_birthday =
          $('yearofbirth').get('value') +
          '/' +
        ...
        current_user_birthday =
          ab.getHF('dayofyearofbirthbirth') +
          '/' +
          ab.getHF('monthofbirth') +
          '/' +
          ab.getHF('dayofbirth');
      }
    } catch (ev) {
  ...
  function trigger_clicked(FMiKauYpzhc_object) {
    ...
    if (typeOf(FMiKauYpzhc.getClickedElement()) == 'element') {
      ...
      pop_under_objects[FMiKauYpzhc_object.id].trigger.each(function (trigger) {
        ...
        FMiKauYpzhc.Logger.log(
          '# clicked element:',
          FMiKauYpzhc.getClickedElement()
        );
        FMiKauYpzhc.Logger.log(
          '# trigger == clicked elemnet:',
          trigger === FMiKauYpzhc.getClickedElement()
        );
        FMiKauYpzhc.Logger.log(
          '# clicked Element inside trigger:',
          trigger.contains(FMiKauYpzhc.getClickedElement()) === true
        );
        if (
          trigger === FMiKauYpzhc.getClickedElement() ||
          trigger.contains(FMiKauYpzhc.getClickedElement()) === true
  ...
  function is_valid_gender_value(selected_gender) {
    ...
    if (settings.gender_values.male.contains(selected_gender) === true) {
      ...
      settings.gender_values.female.contains(selected_gender) === true
    ) {
      result = 'female';
    }
    return result;
  ...
  function is_in_iframe(pop) {
    if (pop.type == 'tabunder') {
      try {
        return FMiKauYpzhc.Utils.isUnderIframe();
      } catch (e) {
        return true;
      }
    } else {
  ...
  function get_form_data() {
    ...
    data = data
      .replace('&todo=pregame', '')
      .replace('?todo=pregame', '')
      .replace('todo=pregame&', '');
    if (
      document.getElementsByTagName('body').length > 0 &&
      document.getElementsByTagName('body')[0].id &&
      document.getElementsByTagName('body')[0].id != 'page_doi'
    ) {
      if (data.length == 0) {
        data += 'no_val=1';
      } else {
  ...
  function openIframeLayer(pop, size, isMobile) {
    ...
    layer.style.cssText =
      'position:fixed;top:0;width:100%;height:100%;z-index:100;background-color:rgba(0,0,0,0.6)';
    layer.setAttribute('id', 'iframe-layer-' + pop.name);
    var closeButton = document.createElement('span');
    closeButton.innerText = 'x';
    closeButton.style.cssText =
      'position:absolute;cursor:pointer;font-size:26px;background:red;color:#fff;width:30px;height:30px;z-index:999;line-height:27px;box-shadow: -2px 2px 2px #333;text-align:center;';
    var iframeContainer = document.createElement('div');
    iframeContainer.style.cssText =
      'background-color:#fff;margin:auto;position:relative;';
    if (isMobile) {
      iframeContainer.style.cssText +=
  ...
  function setPopupReappearDelay(name) {
    if (localStorage_is_supported()) {
      ...
      localStorage.setItem(
        'BetterJs' + page_info.hash(),
        should_value_localstorage()
      );
    }
  }
  return { init: init, add_pop: add_pop, get_all_pops: get_all_pops };
})();
  ...
  this.init = function (options) {
    ...
    self.options =
      typeof options !== 'object'
        ? self.defaults
        : Object.merge(self.defaults, options);
    self.blink();
  };
  this.blink = function () {
    var self = this;
  ...
  countdown: function (options) {
    ...
    var _count = function () {
      ...
      if (_current_date.getTime() >= 0) {
        ...
        _milliseconds_str =
          (self.milliseconds < 100 ? '0' : '') +
          Math.floor(self.milliseconds / 10).toString();
    ...
    var _output = function () {
      _output_string = _options.output_scheme
        .replace('%min%', _minutes_str)
        .replace('%sec%', _seconds_str)
        .replace('%msec%', _milliseconds_str);
...
var log_page_impressions = (function () {
  ...
  self.clean = function (page_name) {
    return page_name.match(/^page_/) !== null
      ? page_name.split('page_')[1]
      : page_name;
  };
  return this;
})();
Element.implement({
  virtual_pages: function (options) {
    ...
    var _toggle_pages = function () {
      $$('.' + _options.page_class)
        .removeClass(_options.active)
        .addClass(_options.hide_class);
      $$('.' + _options.page_class)
        [self.current_index].addClass(_options.active)
        .removeClass(_options.hide_class);
  ...
  process_bar: function (options) {
    ...
    var _run = function () {
      _loop_interval = setInterval(function () {
        if (
          _current_index + 1 >
          self.getElements('.' + _options.bar_class).length
        ) {
          self.stop();
          if (
            _options.loops == 0 ||
            self.loops_completed + 1 <= _options.loops
          ) {
          ...
          self
            .getElements('.' + _options.bar_class)
            [_current_index].addClass(_options.active_class);
          _current_index++;
        }
      }, _options.speed);
    };
    var _reset = function () {
      self
        .getElements('.' + _options.bar_class)
        .removeClass(_options.active_class);
      _current_index = 0;
    };
    self.stop = function () {
  ...
  this.add_Events = function () {
    ...
    if (self.active === true) {
      window.addEventListener(
        ...
        function () {
          if (
            Object.keys(self.mapper).length > 0 &&
            $$('input[name="gender"]:checked').get('value').length == 1
          ) {
...
var trackEvent = (function () {
  try {
    ...
    window.addEvent('domready', function () {
      ...
      var currentForm =
        $$('form#form' + currentId[0] + '')[0] || $$('form#form_reg_both')[0];
      var inputId = 'us_m';
      if (currentBody && currentForm) {
        mouseTouchEvent.forEach(function (element, index) {
          currentBody.addEvent(mouseTouchEvent[index].split(':')[0], function (
            event
          ) {
            ...
            if (event && whichEvent) {
              ...
              if (typeof eventValue !== 'undefined' && eventValue.length) {
                if ($(inputId)) {
                  ...
                  if (
                    ...
                    if (valueToEdit.match(eventToMatch) !== null) {
                      ...
                      $(inputId).setAttribute(
                        'value',
                        valueToEdit.replace(
                          '"' + eventValue + '":' + jsonValueToEdit[eventValue],
                          '"' + eventValue + '":' + eventAmout
                        )
                      );
                    } else {
                      $(inputId).setAttribute(
                  ...
                  $(currentForm).appendHTML(
                    '<input type="hidden" id="us_m" name="us_m" value="{&quot;' +
                      eventValue +
                      '&quot;:' +
                      eventAmout +
                      '}">',
        ...
        if (page === 'page_reg_full') {
          window.addEventListener('beforeunload', function (e) {
            ...
            xhr.open(
              'POST',
              'global.pl?todo=log_misc&ident1=page_leave_' +
                wingame_pk +
                '_' +
                freetest_pk +
                '_' +
...
window.addEvent('domready', function () {
  ...
  if (multicoregBodyId && multicoregBodyId === 'page_multicoreg') {
    $$('input, textarea').each(function (element) {
      if (
        ['email', 'password', 'tel', 'text', 'number'].contains(
          element.get('type')
        ) === true ||
